...
namespace
{
    ...
    void PrintInfo (const fs::path& path)
    {
        ...
        {
            auto gltfStream = streamReader->GetInputStream (pathFile.generic_string().c_str()); // Pass a UTF-8 encoded filename to GetInputString
        ...
        {
            auto glbStream = streamReader->GetInputStream (pathFile.generic_string().c_str()); // Pass a UTF-8 encoded filename to GetInputString
